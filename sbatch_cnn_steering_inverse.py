...
def get_steering_inv(inv_diffeo, layer_shapes):
  down_sampled_inv_diffeo = []
  for layer_shape in layer_shapes:
    x, y = layer_shape
    down_sampled_inv_diffeo.append(inv_diffeo.up_down_sample(x, y, align_corners=True))
...
def inv_diff_hook(inverse_diffeo, batch_size = None):
    ...
    def hook(module, input, output):
        ...
        if batch_size != None:
            ...
            output = t.cat([output, inverse_diffeo(ref, in_inference = True, mode = 'nearest')], dim = 0)
            return output
    return hook

def steering_hooks(layers, inv_diffs):
  hooks = []
  for layer, inv_diff in zip(layers, inv_diffs):
    hooks.append(layer.register_forward_hook(inv_diff_hook(inv_diff, batch_size = len(diffeo_strengths) * num_of_diffeo)))
  return hooks
