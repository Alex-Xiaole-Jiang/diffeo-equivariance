...
layer_num_list = range(14,44)

read_path = '/vast/xj2173/diffeo/scratch_data/all_cnn_layers/ENV2_s/'
ref_path =  '/vast/xj2173/diffeo/scratch_data/all_cnn_layers/ENV2_s/reference/'
# save_path = '/vast/xj2173/diffeo/process_data/all_cnn_layers/cosine_similarity/'
save_path = '/vast/xj2173/diffeo/process_data/all_cnn_layers/inv_cos_sim_extended/'

inv_grid = t.load('/vast/xj2173/diffeo/scratch_data/inv_grids/14-50-4-4-3-224-224_inv_grid_sample.pt', map_location= t.device('cpu'))
...
for counter, (file_path_list, ref_path_list) in enumerate(zip(data_dir_list, ref_dir_list)):
    ...
    for file_path, ref_file_path in zip(file_path_list, ref_path_list):
        ...
        un_diff = (down_sampled(raw_data.reshape(14, 50, channels, x_res, y_res)))
        activation.append(F.normalize(un_diff.reshape(14, 50, channels, x_res * y_res), dim = -1))
        # activation.append(F.normalize(raw_data.reshape(14, 50, channels, x_res * y_res), dim = -1))
