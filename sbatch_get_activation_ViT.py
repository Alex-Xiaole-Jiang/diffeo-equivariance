...
ViT = tv.models.vit_b_16(weights = tv.models.ViT_B_16_Weights.DEFAULT).to(device)
# ViT = tv.models.efficientnet_v2_s().to(device) # random initialization
ViT.eval();
...
print('diffeo & inverse computed')

save_path = 'vast/diffeo/scratch_data/all_ViT_layers/ViT_b_16/'
ref_path = 'vast/diffeo/scratch_data/all_ViT_layers/ViT_b_16/reference/'
...
for i in range(50):
  ...
  activation = retrieve_layer_activation(ViT, distorted_list, layers)
  ref_activation = retrieve_layer_activation(ViT, val_image, layers)
